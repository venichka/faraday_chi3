...
- Probe pulse duration is **not** stated as fixed; if it can be lengthened, Q_s becomes a stronger lever (see §8).
...
## 3. Design levers, ranked **for fixed 100 fs pumps** (§0)
...
**Search:** multi-fidelity — proxy as stage A/B over (r, N, L, material placement, defect length, maybe a
dual-band split for Strategy B); full nonlinear |θ| (three-θ readout) on the top-k as stage C; 3D on the winner.
...
- **Pump pulse duration — RESOLVED: fixed at ~100 fs** (not changeable). ⇒ pump Q bandwidth-capped (§0); the
  whole §3 ranking and the §5 B_i = min(Q, Q_cap) clamp follow from this.
- **Probe pulse duration — OPEN.** Not stated as fixed. If it can be lengthened, Q_s becomes a stronger lever
  (§3.4) and the proxy's w2 should rise; if also ~100 fs, the probe readout is the resonant-slice carrier.
- **Q_cap value** — compute from the actual 100 fs Meep source bandwidth per band (≈ λ/Δλ_pump); set it once.
